...
action_probs_chosen = tf.gather_nd(
    action_probs, action_probs_chosen_indices_ph)
...
delta = tf.reduce_mean(rew_ph + gamma * state_value_tp1 *
                       (1.0 - done_ph) - state_value_t)
...
temp1 = tf.square(rew_ph + gamma * state_value_tp1 *
                  (1.0 - done_ph) - state_value_t)
strain_op = tf.train.GradientDescentOptimizer(
    learning_rate_state).minimize(temp1)
...
with np.errstate(all="raise"):
    for i_episode in range(num_episodes):
        ...
        for t in range(200):
            # perform action
            evaluated_action_probs = sess.run(
                action_probs, feed_dict={obs_ph: [state]})
